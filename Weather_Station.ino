...
void loop(){
  ...
  float pres = (bme280.getPressure() / 100.0)*0.7500637; //hPa
  ...
  Serial.print(vcc);
  Serial.println("mV");
  Blynk.virtualWrite(5, hi); // virtual pin 5
...
long computeDewPoint(float temp, float humi) {
  ...
  dewpoint = c * (log(humi/100.0) + ((b * temp)/(c + temp)))
  /(b - log(humi/100.0) - ((b * temp)/(c + temp)));
